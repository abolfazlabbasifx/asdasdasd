...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'yes' then
    return 'کل تنظیمات از قبل قفل بودند'
  else
    data[tostring(target)]['settings']['all'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'کل تنظیمات قفل شدند'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  if group_all_lock == 'no' then
    return 'کل تنظیمات ققل نیستند'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return 'کل تنظیمات باز شدند'
  end
end
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'yes' then
    return 'تنظیمات اتحاد از قبل قفل است'
  else
    data[tostring(target)]['settings']['etehad'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تنظیمات اتحاد قفل شد'
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'no' then
    return 'تنظیمات اتحاد قفل نیست'
  else
    data[tostring(target)]['settings']['etehad'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تنظیمات اتحاد باز شد'
  end
end
...
local function lock_group_username(msg, data, target)
  ...
  if group_username_lock == 'yes' then
    return 'نام کاربری از قبل قفل است'
  else
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return 'فایل های صوتی از قبل قفل اند'
  else
    data[tostring(target)]['settings']['media'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فایل های صوتی قفل شدند'
  end
end
...
local function unlock_group_media(msg, data, target)
  ...
  if group_media_lock == 'no' then
    return 'فایل های صوتی قفل نیستند'
  else
    data[tostring(target)]['settings']['media'] = 'no'
    save_data(_config.moderation.data, data)
    return 'فایل های صوتی باز شدند'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'حساسیت به اسپم از قبل قفل است'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'حساسیت به اسپم قفل شد'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'حساسیت به اسپم قفل نیست'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'عربی/فارسی از قبل باز است'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'عربی/فارسی باز شد'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'افزودن اعضای سوپر گروه از قبل قفل است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'افزودن اعضای سوپر گروه قفل شد'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'افزودن اعضای سوپر گروه قفل نیست'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'افزودن اعضای سوپر گروه باز شد'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'ار تی ال از قبل قفل است'
  else
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'ار تی ال از قبل باز است'
  else
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'تی جی سرویس از قبل قفل است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تی جی سرویس قفل شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'تی جی سرویس قفل نیست!'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تی جی سرویس باز شد'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'ارسال استیکر از قبل قفل است'
  else
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'ارسال استیکر از قبل باز است'
  else
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'افزودن ربات از قبل قفل است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'افزودن ربات قفل شد'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'افزودن ربات از قبل باز است است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return 'افزودن ربات باز شد'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'ارسال مخاطبین از قبل قفل است'
  else
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return 'حالت سخت تنظیمات از قبل فعال است'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تنظیمات الان رو حالت سخت میباشد'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تنظیمات الان رو حالت سخت نمیباشد'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'قوانین سوپر گروه تنظیم شد'
end
...
local function set_public_membermod(msg, data, target)
  ...
  return 'سوپر گروه الان عمومی است'
end
...
local function unset_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'سوپرگروه الان عمومی نیست'
  end
end
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'سوپر گروه افزوده نشده است.'
  end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "شما نمی توانید کمک ادمین ها/صاحب گروه/ادمین هارو اخراج کنید")
    end
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "شما نمی توانید ادمین های دیگر رو اخراج کنید")
    end
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "شما نمی توانید کمک ادمین ها/صاحب گروه/ادمین ها رو اخراج کنید")
    end
    if is_admin2(member_id) then
	...
	elseif get_cmd == "setleader" then
		...
		if result.from.username then
			text = "@"..result.from.username.." به مقام ادمین ارتقاع یافت"
		else
			text = "[ "..user_id.." ]به مقام ادمین ارتقاع یافت"
		end
	...
	elseif get_cmd == "demoteleader" then
		...
		if result.from.username then
			...
			text = "[ "..user_id.." ] از مقام ادمین به مقام کاربر تنزیل یافت"
		end
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				...
				text = "[ "..result.from.peer_id.." ] افزوده شد به عنوان صاحب گروه"
			end
			send_large_msg(channel_id, text)
		end
...
local function cb_user_info(extra, success, result)
	...
	--[[if get_cmd == "setadmin" then
		...
		if result.username then
			text = "@"..result.username.." به ادمین ارتقاع یافت"
		else
			text = "[ "..result.peer_id.." ] به ادمین ارتقاع یافت"
		end
			send_large_msg(receiver, text)]]
	if get_cmd == "demoteleader" then
		if is_admin2(result.peer_id) then
			return send_large_msg(receiver, "شما نمی توانید مقام گلوبال ادمین ها رو بگیرید!")
		end
		local user_id = "user#id"..result.peer_id
...
local function run(msg, matches)
	...
	elseif msg.to.type == 'channel'then
		if matches[1] == 'tosuper' then
			...
			return "اینجا از قبل سوپر گروه است"
		end
	end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه از قبل افزوده شده است.', ok_cb, false)
			end
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه افزوده نشده است.', ok_cb, false)
			end
		...
		if matches[1] == "owner" then
			...
			if not group_owner then
				return "مدیری برای این گروه انتخاب نشده است"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
			return "ایدی صاحب این گپ ["..group_owner..']'
		end
		...
		if matches[1] == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '*خطا:من نمی توانم لینک جدید رو بسازم* \nمن سازنده گروه نیستم\n\nلطفا از دستور /setlink استفاده کنید و لینک سوپر گروه رو برای من بفرستید')
		...
		if matches[1] == 'setlink' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'لطفا لینک جدید سوپر گروه رو برای من ارسال کنید'
		end
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "اول یک لینک جدید با دستور /newlink بسازید!\n\nیا اگه من سازنده گروه نیستم از دستور /setlink استفاده کنید و لینک گروهتان را برای من ارسال نمایید"
			end
		...
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					...
					local text = "[ "..matches[2].." ] به عنوان مدیر گروه انتخاب شد"
					return text
				end]]
		...
		if matches[1] == 'promote' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "فقط مدیر گروه/ادمینها میتوانند یک نفر را کمک ادمین کنند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == 'demote' then
			...
			if not is_owner(msg) then
				return "فقط مدیر گروه/ادمینها/سودوها میتوانند مقام یک کمک ادمین را بگیرند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
		...
		if matches[1] == "setabout" and is_momod(msg) then
			...
			return "موضوع گروه تنظیم شد.\n\nبه پروفایل سوپر گروهتون برید و تغیرات را نگاه کنید."
		end
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return 'لطفا عکس جدید سوپر گروه خود را ارسال نمایید'
		end
		...
		if matches[1] == 'clean' then
			...
			if not is_momod(msg) then
				return "فقط مدیران حق پاک کردن دارند"
			end
			if matches[2] == 'modlist' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return 'هیچ ادمینی در این گروه نیست.'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
		...
		if matches[1] == 'setflood' then
			...
			if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 200 then
				return "تعداد حساسیت به اسپم از [1-200]"
			end
			local flood_max = matches[2]
			data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
			save_data(_config.moderation.data, data)
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
			return 'حساسیست به اسپم تغییر یافت به: '..matches[2]
		end
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "سوپر گروه سایلنت "..msg_type.." از قبل فعال میباشد"
				end
			end
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "سومر گروه سایلنت "..msg_type.." از قبل فعال میباشد"
				end
			end
			...
			local msg_type = 'Text'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "سایلنت "..msg_type.." از قبل فعال میباشد"
				end
			end
			if matches[2] == 'all' then
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "سایلنت "..msg_type.."  فعال شد"
				else
					return "سایلنت "..msg_type.." از قبل فعال میباشد"
				end
			end
		...
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'Audio'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "سایلنت "..msg_type.." از قبل غیر فعال میباشد"
				end
			end
			if matches[2] == 'photo' then
			local msg_type = 'Photo'
			...
			local msg_type = 'Documents'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "سالنت "..msg_type.." از قبل غیر فعال میباشد"
				end
			end
			if matches[2] == 'text' then
			local msg_type = 'Text'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "سایلنت متن از قبل فعال است"
				end
			end
			if matches[2] == 'all' then
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "سایلنت "..msg_type.." غیر فعال شد"
				else
					return "سایلنت "..msg_type.." از قبل غیر فعال میباشد"
				end
			end
		...
		if matches[1] == "mute" or matches[1] == "unmute" and is_momod(msg) then
			...
			elseif matches[1] == "mute" or matches[1] == "unmute" and string.match(matches[2], '^%d+$') then
				...
				if is_muted_user(chat_id, user_id) then
					...
					return "["..user_id.."] از لیست سایلنت ها حذف شد"
				elseif is_momod(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] added ["..user_id.."] to the muted users list")
					return "["..user_id.."] به لیست سایلنت ها اضافه شد"
				end
